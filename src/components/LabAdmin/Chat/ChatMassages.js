...
function ChatMassages() {
  ...
  let { id } = useParams();
    ...
    channel.bind('inserted', function(newMassage) {
      if(id == newMassage.user_id && user_id == newMassage.lab_admin_id){
  ...
  useEffect(() => {
    let getUserData = async () => {
      await axios
        .get(`http://localhost:5000/lab-admin/chat/user/${id}`, {
          headers: {
            token: token,
          },
        })
    ...
    getUserData();
      ...
      await axios
        .get(`http://localhost:5000/lab-admin/chat/avatar`, {
          headers: {
            token: token,
          },
        })
    ...
    getLabAdminData();
      ...
      await axios
        .get(`http://localhost:5000/lab-admin/chat/messages/${id}`,{
          headers: {
            token: token,
          },
        })
  ...
  e.preventDefault()
  axios.post('http://localhost:5000/lab-admin/chat/messages/new',
  {
    message: input,
    user_id:id,
  },{
